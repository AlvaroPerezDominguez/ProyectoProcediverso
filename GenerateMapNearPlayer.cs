...
public class GenerateMapNearPlayer : MonoBehaviour {
	...
	private void createChunk(Vector3Int chunkAddress)
	{
		...
		catch (CollapseFailedException)
		{
			...
			return; // Detener la generación de chunks
		}
	...
	private void generatorThread()
	{
		try
		{
			while (true)
			{
				if (map.pieceCount >= LimiteConstruccion.PieceLimit)
				{
					Debug.Log("Se ha alcanzado el límite de piezas. Deteniendo la generación de chunks.");
					return; // Detener la generación de chunks
				}
